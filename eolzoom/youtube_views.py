...
def _get_user_credentials_google(user):
    ...
    try:
        ...
        credentials = get_user_credentials_google(credentials_model.credentials)
...
def get_user_credentials_google(credentials_json):
    ...
    if dt.now() >= dt.strptime(credentials_dict["expiry"], "%Y-%m-%d %H:%M:%S.%f"):
        data = refresh_access_token_oauth2(credentials_dict["refresh_token"], credentials_dict["token_uri"])
        if data:
            credentials_dict["token"] = data['access_token']
            new_expiry = dt.now() + datetime.timedelta(seconds=data['expires_in'])
...
def callback_google_auth(request):
    ...
    try:
        ...
        next_url =  base64.b64decode(state).decode('utf-8')  # decode Studio EOL URL
        ...
        logger.error("Error with Exchange authorization code for refresh and access tokens")
        return HttpResponse(status=400)
...
def create_live_in_youtube(youtube, start_time, title):
    ...
    try:
        ...
        logger.info("An HTTP error {} occurred:\n{}".format(e.resp.status, e.content))
        return None
    except RefreshError:
...
def start_live_youtube(user, meet_id):
    ...
    response = {}
    url = "https://api.zoom.us/v2/meetings/{}/livestream/status".format(meet_id)
    r = requests.patch(
        url,
        data=json.dumps(body),
        headers=headers)
...
def create_livebroadcast(request):
    ...
    livebroadcast_data = create_live_in_youtube(youtube, start_time, request.POST['display_name'])
    if livebroadcast_data is None:
        logger.info("Error in Create live in youtube")
        return JsonResponse(response, safe=False)
    status = update_meeting_youtube(request.user,livebroadcast_data,request.POST['meeting_id'])
    if status:
        response['status'] = "ok"
...
def youtube_validate(request):
    ...
    try:
        ...
        credentials_dict = get_user_credentials_google(credentials_model.credentials)
...
def check_permission_live(youtube, data):
    ...
    """
    try:
        id_live = insert_broadcast(youtube, str(dt.now()), "EOL - Validate permission")
...
def update_livebroadcast(request):
    ...
    id_live = update_live_in_youtube(youtube,start_time,request.POST['display_name'],request.POST['broadcast_id'])
    if id_live:
        response['status'] = "ok"
